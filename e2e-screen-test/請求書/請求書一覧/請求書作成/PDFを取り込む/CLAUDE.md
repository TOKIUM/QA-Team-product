...
| # | テスト名 | カテゴリ |
|---|---------|---------|
| 1-27 | 正常系テスト（分割/リネーム基本表示、モード切替、パンくず、ヘッダー/サイドバー、iframe） | 正常系 |
| 28 | test_異常系_存在しないサブパスにアクセス | 異常系 |
| 29 | test_異常系_XSSペイロードをURLに含めてアクセス | 異常系 |
| 30 | test_異常系_SQLインジェクションをURLに含めてアクセス | 異常系 |
| 31 | test_異常系_モード連続切替で画面が安定する | 異常系 |
| 32 | test_異常系_ブラウザバックでモード切替が正常動作する | 異常系 |
| 33 | test_境界値_分割モードのステッパーが正確に3ステップ | 境界値 |
| 34 | test_境界値_リネームモードのステッパーが正確に2ステップ | 境界値 |
| 35 | test_境界値_分割モードの次へボタンはファイル未選択でdisabledかつクリック不可 | 境界値 |
| 36 | test_境界値_リネームモードの次へボタンはファイル未選択でdisabledかつクリック不可 | 境界値 |
